...
export function HomePage() {
  ...
  return (
    <div className="mx-auto flex min-h-[100dvh] max-w-3xl flex-col justify-center px-5 pb-28 pt-24 text-center">
      <p className="eq text-xs tracking-[0.4em]">{teacher.universe}</p>
      <TripleTitle className="serif mt-4 text-5xl sm:text-7xl">{teacher.title}</TripleTitle>
      <p className="mt-5 text-xl text-mute">{teacher.tagline}</p>
      <p className="mt-3 text-mute">{teacher.supporting}</p>
      <p className="eq mt-6 text-xs tracking-wide text-mute">
      ...
      <Link to="/memories" className="mt-6 text-sm underline-offset-4 hover:underline">
